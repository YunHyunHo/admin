...
export async function POST(request: Request) {
  const user = await getSessionUser();
  if (!user) return NextResponse.json({ message: "로그인이 필요합니다." }, { status: 401 });
  if (!canManageTelegramSettings(user)) return NextResponse.json({ message: "마스터 또는 어드민만 설정할 수 있습니다." }, { status: 403 });
  if (!hasDatabaseUrl()) return NextResponse.json({ message: "DB 연결 환경에서만 사용할 수 있습니다." }, { status: 400 });
  const body = (await request.json()) as { action?: string; domainId?: string; token?: string };
  if (!body.domainId) return NextResponse.json({ message: "도메인 정보가 필요합니다." }, { status: 400 });
  try {
    let message = "";
    if (body.action === "save-token") {
      if (!body.token?.trim()) throw new Error("봇 토큰을 입력해주세요.");
      const username = await saveTelegramBot(user, body.domainId, body.token);
      message = `@${username} 봇이 확인되었습니다. 텔레그램에서 /start를 보내주세요.`;
    } else if (body.action === "connect-chat") {
      const title = await connectTelegramChat(user, body.domainId);
      message = `${title} 채팅방이 연결되었습니다.`;
    } else if (body.action === "test") {
      await sendTelegramTest(user, body.domainId);
